...
void *producer(void *thread_n) {
    ...
    while (i++ < PRODUCER_LOOPS) {
        sleep(rand() % 10);
        value = rand() % 100;
        Csem_wait(&full_sem); // sem=0: wait. sem>0: go and decrement it
        ...
        Csem_post(&empty_sem); // post (increment) emptybuffer semaphore
...
void *consumer(void *thread_n) {
    ...
    while (i++ < PRODUCER_LOOPS) {
        ...
        pthread_mutex_unlock(&buffer_mutex);
        Csem_post(&full_sem); // post (increment) fullbuffer semaphore
...
int main(int argc, int **argv) {
    ...
    Csem_destroy(&full_sem);
    Csem_destroy(&empty_sem);
